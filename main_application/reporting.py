...
def create_file(root_path_scanned=os.getcwd(), path_to_save=os.getcwd(), f_name="dir_report"):
    ...
    """
    print(path_to_save, f_name)
    
    if  os.path.exists(root_path_scanned) and os.path.exists(path_to_save):

        #checks if the filename exist
        is_f_exist = os.path.exists(path_to_save+ "/" + f_name)

        f_name_update = f_name
        #append the n value of file copy
        i = 1 
        while is_f_exist:
            f_name_update = f_name + "(" + str(i) +")"
            is_f_exist = os.path.exists(path_to_save+ "/" + f_name_update)
            i +=1 

        print("Creating " + f_name_update + " at " + path_to_save)
        f_name = path_to_save + "/" + f_name_update
...
def append_to_report(file_object, file ,data):
    ...
    try:
        ...
        print("Error writing to the report. ")
